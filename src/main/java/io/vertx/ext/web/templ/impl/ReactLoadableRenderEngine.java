...
/**
 * ReactLoadableRenderEngine 
 */
public class ReactLoadableRenderEngine implements MessageBackedRenderEngine {
  private final EventBus eventBus;

  // SSR responses with React Loadable have additional arguments
  private final Cache<String, JsonObject> cache = Caffeine.newBuilder().maximumSize(DEFAULT_CACHE_SIZE).build();
  ...
  public ReactLoadableRenderEngine(Vertx vertx) {
  ...
  private void decorateFromMessage(RoutingContext context, JsonObject metaObject, String token, String elementKey,
    ...
    eventBus.send(this.rendererAddress, metaObject, ssr_response -> {
      ...
      JsonObject response = new JsonObject(ssr_response.result().body().toString());
      String processed = decorateRenderedComponent(
        response.getString("DOM"),
        elementKey,
        metaObject.getString("name")
      );

      response.put("DOM", processed);

      if (this.cacheEnabled && !propsKey.isEmpty())
        cache.put(propsKey, response);

      context.put(token, response);
    });
  }
  ...
  public Future render(RoutingContext context) {
    ...
    componentList.parallelStream().parallel().forEach(meta -> {
      ...
      if (this.cacheEnabled && props != null) {
        JsonObject alreadyRendered = cache.getIfPresent(propsKey);
